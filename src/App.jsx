...
function App() {
  ...
  const allowedTabs = useMemo(() => {
    ...
    return ['rules', 'organizations', 'applications', 'players'];
  }, [currentUser]);
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      applyProfile(user); setLoginError(''); setApiError(''); setActiveTab('cabinet');
    } catch {
      ...
      applyProfile(user); setApiError('API недоступне або PostgreSQL ще не запущено. Показано локальні seed-дані.'); setActiveTab('cabinet');
    }
  }
  async function saveProfile(e) { e.preventDefault(); if (!currentUser) return; try { const updated = await apiRequest('/profile', { method: 'PUT', body: JSON.stringify(profileForm) }, currentUser.id); applyProfile(updated); await loadData(updated); } catch (e) { setApiError(e.message); } }
  ...
  async function saveRule(e) { e.preventDefault(); if (!currentUser || currentUser.role !== 'admin') return; try { const body = JSON.stringify(ruleForm); if (editRuleId) await apiRequest(`/rules/${editRuleId}`, { method: 'PUT', body }, currentUser.id); else await apiRequest('/rules', { method: 'POST', body }, currentUser.id); setRuleForm(emptyRule); setEditRuleId(null); await loadData(currentUser); } catch (e) { setApiError(e.message); } }
