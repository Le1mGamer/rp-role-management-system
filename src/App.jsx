...
function App(){
  ...
  const tabs=currentUser ? data.rolePermissions[currentUser.role] || ['dashboard'] : [];
  const visibleTabs=allTabs.filter(([id])=>tabs.includes(id));
  const allowed=tabs.includes(activeTab);
  ...
  async function handleLogin(e){ e.preventDefault(); try{ const user=await apiRequest('/auth/login',{method:'POST',body:JSON.stringify({nickname,password})}); setCurrentUser(user); setLoginError(''); setActiveTab((data.rolePermissions[user.role]||['dashboard'])[0]); }catch(_e){ const user=seedData.users.find(u=>u.nickname.toLowerCase()===nickname.trim().toLowerCase() && u.password===password && u.status!=='banned'); if(!user){ setLoginError(t.loginError); return; } setCurrentUser(user); setApiError('API недоступне або PostgreSQL ще не запущено. Показано локальні seed-дані.'); setActiveTab((seedData.rolePermissions[user.role]||['dashboard'])[0]); } }
  function logout(){ setCurrentUser(null); setLeaderOrg(null); setData(seedData); setQuery(''); }
  async function saveRule(e){ e.preventDefault(); try{ const body=JSON.stringify(ruleForm); if(editRuleId) await apiRequest(`/rules/${editRuleId}`,{method:'PUT',body},currentUser.id); else await apiRequest('/rules',{method:'POST',body},currentUser.id); setRuleForm(emptyRule); setEditRuleId(null); await loadData(currentUser); }catch(e){ setApiError(e.message); } }
  ...
  async function submitApplication(e){ e.preventDefault(); try{ await apiRequest('/applications',{method:'POST',body:JSON.stringify({organizationId:Number(appForm.organizationId),type:appForm.type})},currentUser.id); setAppForm({ organizationId:'', type:'join_organization' }); await loadData(currentUser); }catch(e){ setApiError(e.message); } }
  async function addMember(e){ e.preventDefault(); try{ await apiRequest(`/players/${memberUserId}/organization`,{method:'PATCH',body:JSON.stringify({organizationId:leaderOrg?.id})},currentUser.id); setMemberUserId(''); await loadData(currentUser); }catch(e){ setApiError(e.message); } }
  async function removeMember(userId){ try{ await apiRequest(`/players/${userId}/organization`,{method:'PATCH',body:JSON.stringify({organizationId:null})},currentUser.id); await loadData(currentUser); }catch(e){ setApiError(e.message); } }
  async function createPunishment(e){ e.preventDefault(); try{ await apiRequest('/punishments',{method:'POST',body:JSON.stringify({...punishmentForm,userId:Number(punishmentForm.userId)})},currentUser.id); setPunishmentForm({ userId:'', type:'warning', reason:'', endDate:'' }); await loadData(currentUser); }catch(e){ setApiError(e.message); } }

  if(!currentUser){ const demos=seedData.users.filter(u=>u.status==='active').slice(0,6); return <div className="login-page"><section className="login-card"><div className="brand login-brand"><div className="brand-mark">RP</div><div><h1>{t.appTitle}</h1><p>{t.subtitle}</p></div></div><div className="login-header"><h2>{t.loginTitle}</h2><p>{t.loginSubtitle}</p></div><form onSubmit={handleLogin} className="login-form"><label>{t.nicknameInput}<input value={nickname} onChange={e=>setNickname(e.target.value)}/></label><label>{t.passwordInput}<input type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label>{loginError&&<div className="error-box">{loginError}</div>}<button className="primary-btn">{t.signIn}</button></form><div className="login-tools"><label>{t.language}</label><select value={language} onChange={e=>setLanguage(e.target.value)}><option value="uk">UA</option><option value="en">ENG</option></select></div><div className="demo-box"><h3>{t.demoAccounts}</h3>{demos.map(u=><button key={u.id} onClick={()=>{setNickname(u.nickname);setPassword(u.password)}}><b>{u.nickname}</b><span>{t[u.role]||u.role}</span><small>{u.password}</small></button>)}</div></section></div>; }
  ...
  const rules=data.rules.filter(r=>(r.access==='all'||r.access===currentUser.role||currentUser.role==='admin') && `${r.title} ${r.category}`.toLowerCase().includes(q));
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-mark">RP</div><div><h1>{t.appTitle}</h1><p>{t.subtitle}</p></div></div><div className="user-panel"><span>{t.currentUser}</span><b>{currentUser.nickname}</b><small>{t[currentUser.role]||currentUser.role}</small></div><nav>{visibleTabs.map(([id,Icon])=><button key={id} className={activeTab===id?'active':''} onClick={()=>setActiveTab(id)}><Icon size={18}/>{t[id]}</button>)}</nav><button className="logout-btn" onClick={logout}><LogOut size={18}/>{t.signOut}</button></aside><main className="content"><header className="topbar"><div><h2>{t[activeTab]}</h2><p>{t.subtitle}</p></div><div className="topbar-actions"><label>{t.language}</label><select value={language} onChange={e=>setLanguage(e.target.value)}><option value="uk">UA</option><option value="en">ENG</option></select></div></header>{apiError&&<div className="warning-box">{apiError}</div>}{!allowed&&<div className="error-box">{t.accessDenied}</div>}{allowed&&activeTab!=='dashboard'&&<div className="search-box"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={t.search}/></div>}{allowed&&activeTab==='dashboard'&&<section className="dashboard-grid"><Stat title={t.activeUsers} value={data.users.filter(u=>u.status==='active').length}/><Stat title={t.pendingApplications} value={data.applications.filter(a=>a.status==='pending').length}/><Stat title={t.totalRules} value={rules.length}/><Stat title={t.totalOrganizations} value={data.organizations.length}/><Panel title={t.availableSections}>{visibleTabs.map(([id])=><div className="row-line" key={id}><span>{t[id]}</span><b>✓</b></div>)}</Panel><Panel title={t.recentActivity}>{logs.slice(0,7).map(l=><div className="activity" key={l.id}><b>{l.nickname||usersById[l.userId]?.nickname}</b><span>{l.action}</span><small>{String(l.timestamp)}</small></div>)}</Panel></section>}{allowed&&activeTab==='rules'&&<><RuleForm user={currentUser} form={ruleForm} setForm={setRuleForm} onSubmit={saveRule} editId={editRuleId} onCancel={()=>{setEditRuleId(null);setRuleForm(emptyRule)}}/><Cards items={rules} user={currentUser} edit={r=>{setEditRuleId(r.id);setRuleForm({category:r.category,title:r.title,text:r.text,access:r.access})}} remove={removeRule}/></>}{allowed&&activeTab==='applications'&&<><ApplicationForm user={currentUser} organizations={data.organizations} form={appForm} setForm={setAppForm} onSubmit={submitApplication}/><Applications apps={apps} user={currentUser} t={t} change={setAppStatus}/></>} {allowed&&activeTab==='players'&&<><LeaderMembers user={currentUser} organization={leaderOrg} members={orgMembers} freePlayers={freePlayers} memberUserId={memberUserId} setMemberUserId={setMemberUserId} onAdd={addMember} onRemove={removeMember}/><Table headers={[t.nickname,t.role,t.status,t.level,t.reputation,t.organization]} rows={data.players.map(p=>[p.nickname||usersById[p.userId]?.nickname,t[usersById[p.userId]?.role]||p.role,p.status||usersById[p.userId]?.status,p.level,p.reputation,p.organization||orgsById[p.organizationId]?.name||'-'])}/></>} {allowed&&activeTab==='organizations'&&<Table headers={[t.organization,t.type,t.rating,t.members]} rows={data.organizations.map(o=>[o.name,o.type,o.rating,o.members])}/>} {allowed&&activeTab==='punishments'&&<><PunishmentForm users={data.users} form={punishmentForm} setForm={setPunishmentForm} onSubmit={createPunishment}/><Table headers={[t.nickname,t.type,t.reason,t.period]} rows={data.punishments.map(p=>[p.nickname||usersById[p.userId]?.nickname,p.type,p.reason,`${String(p.startDate)} - ${String(p.endDate)}`])}/></>} {allowed&&activeTab==='logs'&&<Table headers={[t.nickname,t.action,t.timestamp]} rows={logs.map(l=>[l.nickname||usersById[l.userId]?.nickname,l.action,String(l.timestamp)])}/>}</main></div>;
}
function Applications({apps,user,t,change}){ return <div className="table-wrap"><table><thead><tr><th>{t.applicant}</th><th>{t.organization}</th><th>{t.type}</th><th>{t.status}</th><th>{t.submittedAt}</th>{['leader','admin'].includes(user.role)&&<th>Actions</th>}</tr></thead><tbody>{apps.map(a=><tr key={a.id}><td>{a.applicant}</td><td>{a.organization}</td><td>{a.type}</td><td>{t[a.status]||a.status}</td><td>{String(a.submittedAt)}</td>{['leader','admin'].includes(user.role)&&<td><button onClick={()=>change(a.id,'approved')}>Approve</button><button onClick={()=>change(a.id,'rejected')}>Reject</button></td>}</tr>)}</tbody></table></div> }
