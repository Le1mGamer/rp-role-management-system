...
function App() {
  ...
  const allowedTabs = useMemo(() => {
    if (!currentUser) return ['rules', 'organizations', 'cabinet'];
    if (currentUser.role === 'admin') return ['rules', 'organizations', 'applications', 'players', 'admin', 'logs', 'cabinet'];
    if (currentUser.role === 'leader') return ['rules', 'organizations', 'applications', 'players', 'logs', 'cabinet'];
    return ['rules', 'organizations', 'applications', 'cabinet'];
  }, [currentUser]);
    ...
    <div className="site-shell">
      ...
      <header className="glass-header">
        ...
        <div className="header-actions">
          <button className="dark-btn" onClick={() => setActiveTab('cabinet')}>Кабінет →</button>
          <button className="accent-btn" onClick={() => setActiveTab(currentUser ? 'applications' : 'cabinet')}>Подати заявку</button>
        </div>
      </header>
      ...
      <main className="landing-content">
        ...
        {activeTab === 'applications' && <ApplicationsView apps={apps} user={currentUser} organizations={data.organizations} appForm={appForm} setAppForm={setAppForm} submitApplication={submitApplication} change={setAppStatus} t={t} />}
